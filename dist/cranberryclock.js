...
/** Discreet reference-art signature for prior demos, outside the package APIs. */
export function mountClockSignature(canvas) {
  const host = canvas.parentElement;
  if (!host || host.querySelector(".clock-signature")) return () => {};
  const link = document.createElement("a");
  link.className = "clock-signature";
  link.href = "https://cranberryclock.com/";
  link.target = "_blank";
  link.rel = "noopener noreferrer";
  link.title = "A little CranberryClock. Made by CranberryClock.";
  link.setAttribute(
    "aria-label",
    "CranberryClock — visit the creator’s portfolio",
  );
  Object.assign(link.style, {
    ...
    borderRadius: "8px",
    overflow: "hidden",
    border: "1px solid #cf986c55",
    boxShadow: "0 4px 20px #0006",
  });
  const image = document.createElement("img");
  image.src = new URL(
    "./assets/branding/cranberryclock.webp",
    import.meta.url,
  ).href;
  image.alt =
    "Cranberry with an embedded clock and a natural pointed crown; no stem or leaves";
  image.width = 76;
  image.height = 52;
